<?php
    class Payment_recordModel extends Mysql {
        ...
        public function InsertPaymentRecord(String $DNI, int $id_accounting, String $fecha_UP) {
            $this->DNI = $DNI;
            $this->id_accounting = $id_accounting;
        ...
        public function InsertPaymentRecordNotAccounting(String $DNI, int $id_accounting, String $fecha_UP) {
            $this->DNI = $DNI;
            $this->id_accounting = $id_accounting;
            $this->fecha_UP = $fecha_UP;
            ...
            if ($fecha_FC > $this->fecha_UP) {
                //Insert payment
                $Query_Insert_payment = "INSERT INTO payment (estudiante, fecha_pago, valor, periodo, estado, observacion) VALUES (?, ?, ?, ?, ?, ?)";
                $Array_Query_payment = array($this->DNI, $this->fecha_UP, $valor, $periodo, 1, 1);
                $result_insert_payment = $this->InsertMySQL($Query_Insert_payment, $Array_Query_payment);
                ...
                //Insert payment  
                $Query_Insert_payment = "INSERT INTO payment (estudiante, fecha_pago, valor, periodo, estado, observacion) VALUES (?, ?, ?, ?, ?, ?)";
                $Array_Query_payment = array($this->DNI, $this->fecha_UP, $valor, $periodo, 0, 1);
                $result_insert_payment = $this->InsertMySQL($Query_Insert_payment, $Array_Query_payment);
